...
⬇️ 1차 리팩토링 후 클래스 다이어그램 수정 (수정이라 간단하게만 그림)
![Image](https://github.com/user-attachments/assets/defa5221-dbf0-43ae-abfc-a783014d28d8)
...
## ☝️ 24.02.06 수정
### 1차 피드백 및 리팩토링
1. SpicyLevelFactory에서 매운맛 단계 관리 : Map -> Enum<br>
   Enum에 대해 키워드만 들어보고 몰랐다. 이번 기회를 통해 상수 데이터를 배열로 쉽게 관리하는 방법을 알았다.<br>
   [⬆ 참고 사이트] https://inpa.tistory.com/entry/JAVA-%E2%98%95-%EC%97%B4%EA%B1%B0%ED%98%95Enum-%ED%83%80%EC%9E%85-%EB%AC%B8%EB%B2%95-%ED%99%9C%EC%9A%A9-%EC%A0%95%EB%A6%AC
   
2. 상수(static final) : 변수명 대문자로 변경(코드 컨벤션)
   
3. 가독성 있는 변수명을 사용하고자 tbkCnt -> tteokbokkiCount
   <br>다른 몇몇 직관성 떨어지는 객체들의 변수명도 변경 완료
   웬만하면 가독성을 위해 의미를 나타내는 단어로 변수명을 지어야 한다는 건 알고 있었지만 너무 길면 가독성이 떨어지지 않을까 싶어서 줄였는데 긴 게 더 좋다고 한다.
   
4. 상속 관계에서 InputCheck 객체 중복 생성하는 부분 삭제<br>
   자식 클래스에 private이 아닌 부모 클래스의 멤버 변수를 선언할 필요가 없음<br>
   상속에 대한 간단한 개념은 알지만 응용해 본 적이 없어서 변수도 상속된다는 사실을 간과하고 있었다.<br>
   확실히 손으로 직접 코딩해봐야 개념도 확실히 머리에 들어오는 것 같고 이해가 명확해지는 것 같다.
   
5. showMenus(), showToppings()에서 토핑, 떡볶이, 사이드 반복 출력 -> 재사용성을 위해 함수 하나로 묶고 인자 전달<br>
   이를 구현하기 위해 많은 것들이 수정돼야 했다. 2차 상속으로 바뀌는 것들도 좀 많아졌고 이에 따라 메서드 오버라이딩을 제대로 확인해 볼 수 있었다.
   명확하게 메뉴라고 생각했던 것들만 상속으로 받았는데 메뉴스러운, 구성이 Menu 클래스와 동일한 것들은 전부 상속시켜버렸다. 훨씬 재사용성이 높아진 걸 체감할 수 있었다.
   진짜 이 부분도 직접 코딩해 보니 머리 한 대 맞은 기분이다.
   이렇게 하나하나씩 코딩 실력을 향상시키는 거구나, 정말 개념 하나하나 아는 것도 중요하지만 그걸로는 한없이 부족하구나 하는 생각이 함께 들었기 떄문,,,
   화이팅하자~!
   
7. 매운맛 선택 처리에서 NullPointerException 예외처리 해결<br>
    다른 예외 처리는 다 챙겼는데 이걸 못 챙겼다니..!! 아숩..~

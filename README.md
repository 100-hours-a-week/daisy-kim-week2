...
### 1. CLI 비동기 프로그램 구현 예시 찾아보기<br>
   ...
   앱에 들어갈 때마다, 또는 알람으로 룰렛을 돌려서 할인 쿠폰이나 무료 상품을 주는 쿠폰에 시간제한을 걸어 제공하는 쇼핑 앱이다.

   이를 참고하여 프로그램을 실행시킬 때마다 "시간 제한 할인 쿠폰"을 제공하는 엽기 떡볶이 주문 서비스로 주제를 확정 지었다.
...
### 구현
쿠폰 클래스의 쉽게 설계됐듯이 구현도 비교적 수월했다.<br>
정말 1주차 덕을 많이 본 기분이다.<br>

Timer 클래스를 어느 정도 구현하고 실행을 반복하다 보니 할인 쿠폰을 적용하려면 결제 부분을 건드려야 한다는 사실이 보였다.<br>
결제 부분에 가격 업데이트 부분을 어느 정도 구현한 후 다시 Timer, 스레드 부분으로 돌아왔다.<br>
Timer 클래스만 작성했다보니 다른 클래스와의 상호작용을 어떻게 구현해야 할지 생각나지 않았다.<br>
그러다가 얼추 다 구현했다 싶었는데 테스트에 테스트에 테스트를 하다보니 되던 게 안 되고 안 되던 게 왜 되는지 모르겠는데 되고 암튼 난리였다.

1차 시도 - 처음에는 사용자에게 입력 받는 Scanner 역할의 스레드 받는 상속 클래스를 하나 더 만들었다가 아무리 해도 예외 처리가 안 되길래 포기했다.<br>
2차 시도 - 스레드를 중단하는 스레드를 만들었다. 근데 이것도 예외처리 실패<br>
그러다 어떻게 해결할지 모르겠는 오류가 있었는데 몇 번씩 현재 스레드가 주인이 아니라는 오류였다. 나중에 자세히 읽으니 "Thread-main"를 보게 되었다.
...
저거 보고 main과 타이머 스레드 간 상호작용이 이미 있었구나 하는 것도 바로 깨달을 수 있었고<br>
inerrupt() 함수, wait() 함수가 돌아가는 것도 갑자기 확 머리에 들어왔다.<br>
내가 짜놓고 '나 왜 이렇게 구현한 거지', '아니 이거 왜 돌아가지', '아니 어디서 안 되는 거지', 진짜 복잡함의 연속이었는데<br>
갑자기 해결능력 탑재된 것 마냥 하나씩 보여서 해결했다.<br>
처음에 오류 와방 났을 때는 진짜 그냥 답답했는데 오류 하나 보일 때마다 연결되는 부분 전부 보고 해결 순서 로직 종이에 적으면서 건드리다보니
<br>실제로 차근차근 정말 한 단계씩 계단 밟는 기분이 들어서 신기했다.

하나 아쉬운 건 너무 예외 처리, 스레드에 집중해서 과제를 수행했다보니 객체지향적이었는지는 모르겠다.<br>
쿠폰 종류 자체의 구현에서는 팩토리 패턴이 적용됐지만 스레드를 구현하며 결제 클래스를 건드리다 보니 객체지향적인지 확인할 정신까지 없었다...<br>
정신이 없는 것도 있는데 뭔가 구현을 어느 정도 다 하고 결제 부분, 스레드 부분에서 어떻게 객체지향적으로 나눌 수 있을지 감이 잘 안 왔다.<br>
그래도 마지막에 시작적 요소를 위해 LinePrint 클래스를 작성하여 쉽게 보이는 재사용할 수 있는 부분은 최대한 분리하였다.<br>
그리고 scanner 스레드로 시도했을 때 InputCheck 클래스에서 똑같은 함수를 작성할 뻔 헀다가 코드 중복이 너무 심해 보여 생성자 오버로딩으로 구현할 수 있는 방법을 생각했다. <br>
(물론 스레드를 바꾸면서 최종 코드에 반영되지는 않았지만...)<br>
메인 함수도 좀 나누고... ㅎㅎ<br>
나름 생각하며 구현하긴 했는데 결제 부분... 뭔가 코드가 깔끔해 보이지 않는데 정확히 어느 부분이 어떻게 객체지향적이지 않은 건지 모르겠어서 마음에 걸린다...
